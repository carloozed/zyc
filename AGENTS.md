...
- `src/helpers/seo.ts`, `src/app/sitemap.ts`, `src/app/robots.ts`: production origin, locale paths, canonical/hreflang metadata, sitemap and robots. See "SEO".
...
- `/slice-simulator`: Prismic slice simulator route. `noindex` and disallowed in robots.txt.
- `/sitemap.xml` and `/robots.txt`: generated by `src/app/sitemap.ts` and `src/app/robots.ts`. The middleware matcher skips any path containing a dot, so they are never rewritten into the locale route.
...
- Canonical URLs, `hreflang` alternates, the sitemap and robots.txt are generated from `src/helpers/seo.ts`; see "SEO".
...
Files: `src/helpers/seo.ts`, `src/app/sitemap.ts`, `src/app/robots.ts`, the `metadata` export in `src/app/layout.tsx`.

- `SITE_URL` is `https://www.zurichyouthclassical.ch`. The apex domain 308s to www, so www is canonical. The root layout sets `metadataBase` to it, which makes every relative URL in page metadata absolute.
- `localePath(locale, path)` mirrors the middleware: German on bare paths, English under `/en-us`. Use it instead of building locale URLs by hand.
- Every page's `generateMetadata` spreads `alternates: localeAlternates(lang, path)`. That emits the canonical URL plus `hreflang` links for `de-CH`, `en-US` and `x-default` (the German page). Magazine posts pass a path per locale built from the document's `alternate_languages`, so a post that is only published in one locale gets no dangling alternate.
- `sitemap.ts` lists the home page, every single-type page in `SINGLE_PAGES`, and all magazine posts, once per locale, each with the same `hreflang` set and Prismic's `last_publication_date` as `lastmod`. It revalidates hourly and on the Prismic webhook's `revalidateTag('prismic')`.
- `robots.ts` allows everything except `/api/` and `/slice-simulator` and points at the sitemap. Pages that must not be indexed carry `robots: { index: false }` in their metadata instead of a robots.txt disallow, so crawlers can still see the directive: `newsletter_confirmed` and `slice-simulator`.
- Still open: `<html lang>` is hard-coded to `en` in the root layout; the locale wrapper div carries the real `lang`. Search engines mostly rely on `hreflang` and content, but fixing it would need the root layout to learn the locale, for example from a header set by the middleware.
...
- When adding a page backed by Prismic, update Prismic routes (`/:lang/...`), create or update custom types, add metadata, add the route to `EXCLUDED_PAGES` if the scroll indicator makes no sense there, give its `generateMetadata` an `alternates: localeAlternates(lang, '/path')`, add its type and path to `SINGLE_PAGES` in `src/app/sitemap.ts`, and check both locales.
